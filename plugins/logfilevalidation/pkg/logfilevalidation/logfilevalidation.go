...
func (p *Plugin) Open(prms string) (source.Instance, error) {
	...
	data, err := exec.Command("python3", "/usr/share/falco/plugins/botoscript/logfilevalidation.py").Output()
	//fmt.Printf("\nData: %s",data)
	if err != nil {
...
func (m *Plugin) Fields() []sdk.FieldEntry {
	return []sdk.FieldEntry{
		{Type: "string", Name: "lfv.Bucket", Display: "bucket", Desc: "Bucket name"},
		{Type: "string", Name: "lfv.EventName", Display: "event", Desc: "Event Name"},
		{Type: "string", Name: "lfv.LogFileValidation", Display: "key", Desc: "Bucket Key"},
...
func getfieldStr(jdata *fastjson.Value, field string) (bool, string) {
	...
	case "lfv.LogFileValidation":
		val:=jdata.GetBool("LogFileValidation")
		//fmt.Println(val)
                if val == false {
