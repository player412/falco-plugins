...
func (p *Plugin) Open(prms string) (source.Instance, error) {
	...
	data, err := exec.Command("python3", "/usr/share/falco/plugins/botoscript/publicaccessblock.py").Output()
	//fmt.Printf("\nData: %s",data)
	if err != nil {
...
func (m *Plugin) Fields() []sdk.FieldEntry {
	return []sdk.FieldEntry{
		{Type: "string", Name: "pba.Bucket", Display: "bucket", Desc: "Bucket name"},
		{Type: "string", Name: "pba.EventName", Display: "event", Desc: "Event Name"},
		{Type: "string", Name: "pba.BlockPublicAcls", Display: "key", Desc: "Bucket Key"},
		{Type: "string", Name: "pba.Date", Display: "key", Desc: "Message type, e.g. 'star' or 'repository'."},
		{Type: "string", Name: "pba.IgnorePublicAcls", Display: "key", Desc: "Bucket Key"},
		{Type: "string", Name: "pba.BlockPublicPolicy", Display: "key", Desc: "Bucket Key"},
		{Type: "string", Name: "pba.RestrictPublicBuckets", Display: "key", Desc: "Bucket Key"},
		{Type: "string", Name: "pba.LogFileValidation", Display: "key", Desc: "Bucket Key"},
		{
			Type: "uint64",
			Name: "pba.divisible",
...
func getfieldStr(jdata *fastjson.Value, field string) (bool, string) {
	...
	case "pba.BlockPublicAcls":
		val:=jdata.GetBool("BlockPublicAcls")
		//fmt.Println(val)
                if val == false {
	...
	case "pba.IgnorePublicAcls":
                val:=jdata.GetBool("IgnorePublicAcls")
                //fmt.Println(val)
                if val == false {
                        res = strconv.FormatBool(val)
                } else {
	...
	case "pba.BlockPublicPolicy":
		val := jdata.GetBool("BlockPublicPolicy")
		// fmt.Println(val)
		if val == false {
		res = strconv.FormatBool(val)
		} else {
	...
	case "pba.RestrictPublicBuckets":
		val := jdata.GetBool("RestrictPublicBuckets")
		// fmt.Println(val)
		if val == false {
		res = strconv.FormatBool(val)
		} else {
